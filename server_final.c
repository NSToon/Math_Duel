...
void* play_game(void* args_ptr) {
     GameArgs* args = (GameArgs*)args_ptr;
    int client1 = args->client1;
    int client2 = args->client2;
    free(args); 
    srand(time(NULL));
    ...
    send(client1, "Send your total time in s:\n", 28, 0);
    send(client2, "Send your total time in s:\n", 28, 0);
    ...
    double time1 = atof(time_buf1);
    double time2 = atof(time_buf2);


    char final[BUFFER_SIZE];
    snprintf(final, sizeof(final),
        "Final Score:\nClient 1: %d\nClient 2: %d\nClient 1 time: %.3f seconds\nClient 2 time: %.3f seconds\n",
        total_score1, total_score2, time1, time2);

    if (time1 < time2) {
        strcat(final, "Client 1 was faster and gets +3 bonus!\n");
        total_score1 += 3;
    } else if (time2 < time1) {
        strcat(final, "Client 2 was faster and gets +3 bonus!\n");
        total_score2 += 3;
    } else {
...
int main() {
    ...
    while (1) {
        int client1 = accept(server_md, NULL, NULL);
        printf("Client 1 connected\n");

        int client2 = accept(server_md, NULL, NULL);
        ...
        // Allocate args for this room
        GameArgs* args = malloc(sizeof(GameArgs));
        args->client1 = client1;
        args->client2 = client2;

        pthread_t tid;
        pthread_create(&tid, NULL, play_game, (void*)args);
        pthread_detach(tid);  // ทำให้ไม่ต้องรอ thread นี้จบ
    }
